...
class VitalsPatternDetector {
  ...
  static List<Insight> analyzeBp(List<BpPoint> points) {
    final out = <Insight>[];
  ...
  static List<Insight> analyzeGlucose(List<GlucosePoint> points) {
    final out = <Insight>[];
    if (points.length < 3) return out;
...
/// Convenience: which engine produced an insight (all deterministic here).
const AiEngineKind kDeterministic = AiEngineKind.ruleBased;
