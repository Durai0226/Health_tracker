...
class DrugInfoCatalog {
  ...
  static Future<void> ensureLoaded() async {
    ...
    try {
      final raw = await rootBundle.loadString('assets/health/drug_info.json');
  ...
  /// The full monograph for a medicine, or null when it isn't in the catalogue.
  ///
  /// Exposed so the medication detail screen can render a static "About this
  /// medicine" section (uses / side effects / food / precautions) rather than
  /// asking the user to phrase a question first.
  static DrugInfo? find({String? generic, String? displayName}) =>
      _lookup(generic) ?? _lookup(displayName);
